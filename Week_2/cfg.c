...
Node* parse_Factor() {
  ...
  if (current_token.type == TOK_ID || current_token.type == TOK_NUM) {
    ...
    printf("Syntax Error: Expected ID, NUM, '(', or '!', got '%s'\n", current_token.lexeme);
    exit(1);
  }
  return n;
}
...
int main() {
  ...
  if (current_token.type != TOK_EOF) {
    ...
    if (current_token.type == TOK_EOF) {
      ...
      print_leftmost_derivation(ast);
      print_rightmost_derivation(ast);

    } else {
      printf("Syntax Error: Extraneous tokens at end of input. Got '%s'\n", current_token.lexeme);
    }
  }

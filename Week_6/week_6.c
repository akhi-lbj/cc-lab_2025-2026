...
void generate_bool_tac(ASTNode *n, const char *true_label, const char *false_label) {
    ...
    if (n->kind == NODE_BINARY_OP && n->op == TOK_AND) {
        char *mid = new_label();
        generate_bool_tac(n->children[0], mid, false_label);   /* if left false → skip */
        emit("label", "", "", mid);
    ...
    /* ── Logical OR (short-circuit) ──────────────────────────────────── */
        ...
        char *mid = new_label();
        generate_bool_tac(n->children[0], true_label, mid);    /* if left true → done */
        emit("label", "", "", mid);
        generate_bool_tac(n->children[1], true_label, false_label);
        free(mid);
    ...
    /* ── Relational comparison ───────────────────────────────────────── */
        ...
        char cond[128];
        sprintf(cond, "%s %s %s", arg1, op_str, arg2);
